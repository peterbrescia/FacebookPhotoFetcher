...
function fetch_photos(access_token::ASCIIString, save_directory::ASCIIString)
"""
function fetch_photos(access_token::ASCIIString, save_directory::ASCIIString)
...
function fetch_image(
    photo_id::ASCIIString,
    access_token::ASCIIString,
    output::Dict,
)
    url = "https://graph.facebook.com/v2.5/$photo_id?access_token=$access_token&fields=backdated_time_granularity,album,images,name,created_time,picture,link,event,from,name_tags,place,updated_time,width,page_story_id,can_delete,backdated_time,can_tag,height,icon,id,comments,likes,tags"
    ...
    SQLite.query(
        ...
        [photo_id, JSON.json(parsed_response)],
    )
